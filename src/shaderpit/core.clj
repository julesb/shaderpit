...
(defn do-key-movement-2d [state keychar]
  (let [key-movement-map {
          ...
          \1 (fn [s] (update-in s [:brightness] #(- % 0.001)))
          \2 (fn [s] (update-in s [:brightness] #(+ % 0.001)))
          \3 (fn [s] (update-in s [:contrast] #(- % 0.001)))
          \4 (fn [s] (update-in s [:contrast] #(+ % 0.001)))
          \5 (fn [s] (update-in s [:saturation] #(- % 0.001)))
          \6 (fn [s] (update-in s [:saturation] #(+ % 0.001)))
    }]
