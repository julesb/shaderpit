...
vec2 obj_twospheres(vec3 p) {
    vec3 pr = rotate_y(p, PI*0.5);
    return op_union(obj_boxsubsphere(pr-vec3(0.0, 10.0,0.0)),
                    obj_boxsubsphere(p-vec3(0.0, 20.0,0.0)));
}


vec2 distance_to_obj(in vec3 p) {
    vec3 pr = rotate_x(p-vec3(0.0,2.0,0.0), PI*0.5);
    vec2 scene1 = 
        //op_sblend(p, 
        op_union( 
            obj_floor(p),
            
            op_union(obj_box(p-vec3(0.0,30.0,0.0), vec3(32.0,0.5,32.0)),
                     obj_twospheres(p))
            //vec2(sdLoxodrome(pr*1.0, 2.0, 3.0, 0.1)*4.0, 4.0)
...
float iqsoftshadow2( in vec3 ro, in vec3 rd, float ldist, float mint, float maxt, float k ) {
    float res = 1.0;
    for( float t=mint; t < maxt; ) {
        float h = distance_to_obj(ro + rd*t).x;
        if( h < 0.01 && length(rd*t) < ldist)
            return 0.0;
        res = min( res, k*h/t );
        t += h;
    }
...
void main(void) {
    ...
    */
    ...
    vec3 lightpos = cam_pos + normalize(vpn)*10.0; // + u*2.0 + v*2.0;
...
*/
    ...
    if (f < maxd) {
        ...
        //gl_FragColor=vec4(nsteps,nsteps,nsteps,1.0);
    }
    else {
